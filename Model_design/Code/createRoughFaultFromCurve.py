...
parser.add_argument('--addRoughness',  dest='addRoughness', action='store_true' , default = False, help='add fault roughness to the fault, parametrized by roughness.ini')
...
for i in range(0,nx):
   if args.constExtrudeDir==None:
      ...
      if args.constExtrudeDir[0]==360:
         ...
         strike = args.constExtrudeDir[0]*pi/180.
         v0 = [-cos(strike), -sin(strike), 0.]
   v0[2] = 0
...
if args.addRoughness:
   ...
   config.read('roughnessParam.ini')
   ...
   from generateRoughFault import *
   X,Y,h = GenerateRoughFault(Ls=xi[nNewNodes-1]+xi[1], Ld=yi[nd-1]+yi[1], lambdaMin=lambdaMin, lambdaMaxS=lambdaMaxS, lambdaMaxD=lambdaMaxD, alpha=pow(10, alphaExp), distribution=distribution)
   print(np.percentile(h,90))
   print(np.shape(h),np.shape(X), np.shape(Y))
   startTime = datetime.now()
   f = interpolate.RectBivariateSpline(X, Y, np.transpose(h), kx=1, ky=1)
