function my_plot(obs_t, data_obs_d_f, syn_t, data_syn_d_f,max_obs_t,max_syn_t, station, chan, shift,j,k )
     ...
     plot(obs_t-shift,data_obs_d_f/max(abs(data_obs_d_f)),'r','LineWidth', 1.2);
     if(j==1)
     ...
     if(k==1)
        text(-5,-1,['\bf' '\it' station],'FontSize', 18);
    end
...
hold on
     ...
     plot(syn_t,data_syn_d_f/max(abs(data_syn_d_f))-2,'k','LineWidth', 1.2); 
